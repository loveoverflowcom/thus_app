...
Future<void> registerStorageModule(GetIt getIt) async {
  if (!getIt.isRegistered<HiveInitializer>()) {
    getIt.registerLazySingleton<HiveInitializer>(() => HiveInitializer(logger: getIt<AppLogger>()));
  }

  // Example factories for common caches can be added here.
  if (!getIt.isRegistered<CacheRepositoryFactory>()) {
    getIt.registerLazySingleton<CacheRepositoryFactory>(() => CacheRepositoryFactory(getIt<HiveInitializer>()));
  }
}
...
class CacheRepositoryFactory {
  ...
  CacheRepository<T> box<T>(String name, {bool encrypted = false}) =>
      CacheRepository<T>(_initializer, boxName: name, encrypted: encrypted);
}
